...
<template>
  <div ref="Project" class="project">
    <div ref="ProjectBg" class="project-bg" :style="pickupEndData ? `background-color:${pickupEndData.siteColor.bodyContentsColor};` : ''">
              ...
              <div v-for="(data, index) in projectData" :key="data.id">
                <div class="project-card-item" :class="`project-card-item-0${index}`">
                  <AppCard
                    ...
                    :desc="data.desc"
                    :rotate="data.projectNum?.rotate || 0"
                    :xspeed="data.projectNum?.xspeed || 0"
                    :yspeed="data.projectNum?.yspeed || 0"
                  />
                </div>
              </div>
              <div>
...
export default {
    ...
    ProjectAnimationState: function () {
      switch (this.ProjectAnimationState) {
        ...
        case 'end':
          ...
          setTimeout(() => {
            this.$refs.ProjectBg.style.backgroundColor = this.pickupEndData?.siteColor.bodyContentsColor || '#f0efeb'
          }, 300)
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      if (this.$refs.ProjectCardInner) {
        this.projectCardHeight = this.$refs.ProjectCardInner.clientHeight
      }
    })
  },
