<template>
  <div ref="Pickup" class="pickup is-pickup-scene-00">
    ...
    ></span>
    <span v-if="pickupData.length > 1" class="pickup-circle-bg-area">
      <AppCircleBg 
        v-if="pickupData[1]"
    ...
    <!--シーンごとのセクションナンバー-->
    <p v-if="pickupData.length > 0" class="pickup-section-number">
    ...
    <!--シーンごとのセクションテキスト-->
    <p v-if="pickupData.length > 0" class="pickup-section-text">
      <span
      ...
      >
        ...
        <template v-if="data.pickup.text && data.pickup.text[0]">
    ...
    <!--シーンごとのセクションタイトル-->
    <div v-if="pickupData.length > 0" class="pickup-bg">
            ...
            <h2 class="pickup-title">
              <template v-if="pickupData.length > 0">
                <span v-for="(data, index) in pickupData" :key="data.id" class="pickup-title-wrapper" :class="`pickup-title-wrapper-0${index + 1}`" :style="`color:${data.siteColor.allTextColor};`">
                  <template v-if="data.pickup.title && data.pickup.title[0]">
