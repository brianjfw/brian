<template>
  <div ref="Pickup" class="pickup is-pickup-scene-00" v-if="pickupData && pickupData.length > 0">
    <!--背景サークルー(侵入時の背景サークルとDOMを分ける)-->
    <span ref="PickupCircleEnter" class="pickup-circle-bg-enter" :class="{ 'is-enter': indexPickupState }" :style="`background-color:${pickupData[0]?.siteColor?.bodyContentsColor || '#ffffff'};`"></span>
    <span class="pickup-circle-bg-area">
      <AppCircleBg :state="isCircleBgState02" :modifier="'pickup-02'" :color="pickupData[1]?.siteColor?.bodyContentsColor || '#ffffff'" />
      <AppCircleBg :state="isCircleBgState03" :modifier="'pickup-03'" :color="pickupData[2]?.siteColor?.bodyContentsColor || '#ffffff'" />
    </span>
    <!--シーンごとのセクションナンバー-->
...
import AppCircleBg from '../AppCircleBg.vue'
...
export default {
  ...
  mounted() {
    ...
    // Wait for ASScroll to be ready using event bus
